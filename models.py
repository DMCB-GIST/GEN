...
class Tokenizer():
    def __init__(
        ...
        self.entre_to_id = collections.OrderedDict([(ENTREZID, ID) for ENTREZID, SYMBOL,ID in self.Gene_vocab.values])
        self.symb_to_id = collections.OrderedDict([(SYMBOL, ID) for ENTREZID, SYMBOL,ID in self.Gene_vocab.values])
    ...
    def convert_entre_to_id(self, entres):
        return [self.entre_to_id.get(str(entre)) for entre in entres]
            
    def convert_symb_to_id(self, symbs):
        return [self.symb_to_id.get(symb) for symb in symbs]

    def convert_id_to_entre(self, indices):
        return [list(self.entre_to_id.keys())[list(self.entre_to_id.values()).index(index)] for index in indices]
    ...
    def encode(self, sample, add_special_tokens = True, 
        ...
        if gene_type == 'ENTREZID':
            token['genes'] = self.convert_entre_to_id(token['genes'])
        else:
    ...
    def encode_pair(self, sample1, sample2, add_special_tokens = True, 
        ...
        if gene_type == 'ENTREZID':
            token1['genes'] = self.convert_entre_to_id(token1['genes'])
            token2['genes'] = self.convert_entre_to_id(token2['genes'])
        else:
