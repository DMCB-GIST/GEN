...
if not personalized_genes:
    ...
    title = 'Fixed_Additional_'+name+'_Adim_'+str(att_dim)+'_Ddim_'+str(dim_drug)+'_nGenes_'+str(nGenes)+'_GNN_do'+str(gnn_dropout)+'_att_do_'+str(att_dropout)+'_lr_'+str(lr)
...
IC = pd.read_csv('./data/binary_PANCANCER_IC_82833_580_170.csv')
...
for train1_index, test1_index in split1:
    ...
    for train2_index, test2_index in split2:
        ...
        loss_test = sum_loss/count #log_loss(all_y, all_pred_y)
        AUC, AUPR, F1, ACC, Recall, Specificity, Precision = metrics_graph(all_y,all_pred_y)
