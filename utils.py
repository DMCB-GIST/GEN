...
def get_drug_cell_info(Drug_info_file,Drug_feature_file,Gene_expression_file,
    ...
    if small_genes:
        ...
        gexpr_feature = pd.read_csv(Gene_expression_file,sep='\t',header=0,index_col=[0],engine='python').T
        
    
    if threshold:
        selector = VarianceThreshold(threshold)
        selector.fit_transform(gexpr_feature).shape
        gexpr_feature = gexpr_feature[gexpr_feature.columns[selector.get_support(indices=True)]].T
    else:
    ...
    gexpr_feature.columns = gexpr_feature.columns.astype(int)
    
    cancer_response_exp_file = './data/GDSC_IC50.csv'
    experiment_data = pd.read_csv(cancer_response_exp_file,sep=',',header=0,index_col=[0],engine='python').T
    
    common_indices = (set(gexpr_feature.columns).intersection(set(experiment_data.columns)))
    
    gexpr_feature = gexpr_feature[common_indices].T
    experiment_data = experiment_data[common_indices]
...
def get_idx(drugid2pubchemid, drug_pubchem_id_set, gexpr_feature,experiment_data,binary=False):
    ...
    drug_match_list=[item for item in experiment_data.index if item in drugid2pubchemid.keys()]
...
def get_rand_genes(tokenizer, x_samples, nGenes, gexpr_feature):
    ...
    rand_ids = torch.tensor([tokenizer.convert_entre_to_id(genes) for genes in rand_genes])
...
def get_binary_gene_set(tokenizer, gexpr_feature, nGenes, random_genes = True, num_augment = 1):
    ...
    for i in range(abs_gexpr.shape[0]):
        ...
        for j in range(num_augment):
            ...
            ids = tokenizer.convert_entre_to_id(genes)
